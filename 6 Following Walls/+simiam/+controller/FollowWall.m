...
%             obj.p = simiam.util.Plotter();
        ...
        function outputs = execute(obj, robot, state_estimate, inputs, dt)
            ...
            d_fw = inputs.d_fw;         % defined in QBSupervisor.m
            ...
            if(strcmp(inputs.direction,'right'))
                ...
                sensors_right = ir_distances(3:5);
                [sensors_right, idx] = sort(sensors_right);   % idx(1) and idx(2) are the smallest
                
                p_1 = ir_distances_wf(:,idx(1));
                p_2 = ir_distances_wf(:,idx(2));
            else
                % Pick two of the left sensors based on ir_distances
                sensors_left = ir_distances(1:3);
                [sensors_left, idx] = sort(sensors_left);   % idx(1) and idx(2) are the smallest
